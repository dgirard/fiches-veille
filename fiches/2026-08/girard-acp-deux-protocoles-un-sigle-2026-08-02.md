...
- **Date / source** : **2 août 2026**, note de veille de **Didier Girard**.
- **Cadrage clé** : trois protocoles se disputent le sigle ACP, sans aucune intersection technique, et la ressemblance de surface est maximale (« protocole », « agents », « open source », « 2025-2026 »).
...
Analogie qui porte : ACP #1 est à l'agent ce que **LSP** est au langage — avant, N éditeurs × M agents = N×M intégrations sur mesure ; après, N+M suffit. Et la distinction avec MCP tient en deux lignes : **MCP** relie un agent à ses **outils et données**, **ACP** relie un **client** à un **agent** ; les deux s'empilent.

### La règle de curation, vrai livrable de la note

*« On n'indexe jamais un sigle seul. »* L'entité canonique est le **nom complet** ; le sigle n'est qu'un **alias**, et un alias peut être porté par plusieurs entités. Directement applicable à ce dépôt — voir [`docs/reference/ontologie-kg.md`](docs/reference/ontologie-kg.md) et `scripts/entity_aliases.tsv`.

### Vérification menée sur ce corpus le 2 août 2026

| Affirmation de la note | Résultat |
|---|---|
| *« Zéro sur le protocole de Zed »* | **confirmé** — la chaîne « Agent Client Protocol » était absente de `fiches/` et de `kb/` ; lacune comblée le jour même par [[agentclientprotocol-introduction-2026-08-02]] |
| *« Douze résultats »* | **non reproductible au grep** — 3 fiches contiennent le token `ACP`, pour 32 occurrences. Le « douze » vient vraisemblablement d'un moteur sémantique. Le chiffre ne se vérifie pas ; la conclusion qualitative, si |
| Le piège était-il seulement possible ? | **déjà armé** — le sigle nu `ACP` était typé comme **entité** dans le graphe, sujet de triples dans deux fiches, à côté d'une variante `Agentic Commerce Protocol (ACP)`, soit trois entités pour un même protocole |
| Défaut matérialisé | la page `kb/Agentic-Commerce-Protocol.md` portait *« catégorie : Protocole commerce agentique **Google** »* et la relation `Google a_créé → Agentic Commerce Protocol` (confiance 0,99), alors que l'ACP est **OpenAI + Stripe** et que le protocole de Google est **UCP** (+ AP2) |

La collision décrite avait donc déjà produit une **erreur d'attribution** dans le graphe. Corrigé le 2 août : variantes normalisées vers `Agentic Commerce Protocol`, et symétriquement `UCP` → `Universal Commerce Protocol`, `ACS` → `Agentic Commerce Suite`. La règle n'était pas préventive ici, elle était corrective.

### Gouvernance d'ACP #1, nuance à apporter

La note écrit *« Zed a lancé la chose, puis l'a laissée partir »*. Le mouvement d'ouverture est réel et vérifiable (organisation propre, spécification versionnée, registre public, implémentation JetBrains), mais les sources publiques décrivent une **gouvernance partagée Zed × JetBrains** — registre co-lancé en janvier 2026 —, ACP restant présenté côté Zed comme son standard ouvert et la fonctionnalité phare de Zed 1.0. Ouverture, oui ; dépossession, non. Détail infirmé depuis par la source primaire : le numéro **ACP 1.2** n'est pas corroboré, la navigation de la spécification exposant **`v1` (Latest)** et **`v2` (Draft)**. L'URL `cdn.agentclientprotocol.com/registry` reste non confirmée.

### Buzz, le cas concret — vérifié et exact

Publié par **Block** le **21 juillet 2026**, Apache-2.0. Espace de travail auto-hébergeable sur **Nostr** ; chaque participant humain ou agent est une **paire de clés** (Schnorr) ; chaque message, étape de workflow et push git est un **événement signé** dans un journal append-only. Trois briques : **`buzz-acp`** (harnais traduisant les événements Buzz vers un agent, en ACP sur stdio), **`buzz-agent`** (agent ACP appelant un LLM et utilisant des outils MCP), **`buzz-dev-mcp`** (serveur MCP shell + édition). Trois harnais livrés d'origine : **Goose** (Block), **Claude Code** (Anthropic), **Codex** (OpenAI).

Ce n'est pas « un produit qui utilise ACP » mais la démonstration que le découplage tient : trois agents de trois éditeurs concurrents branchés par le même harnais, et un quatrième — **Hermes** (Nous Research) — raccordé sans que Block écrive une ligne. La promesse N+M devient un fait observable. Nuance : côté Hermes, la source publique cadre l'ajout comme un **mode serveur ACP** ouvrant Zed, JetBrains et Neovim, le bénéfice pour Buzz étant une conséquence d'ACP plutôt qu'une intégration ciblée — ce qui renforce l'argument.

Lecture d'architecture d'entreprise : Buzz donne un substrat multi-agent souverain et auditable, sans dépendance SaaS tierce — journal signé, relay chez soi, identités cryptographiques. C'est l'option auto-hébergée du besoin de portabilité analysé dans [[janakiram-agent-platform-portability-contract-2026-07-20]].

### Abonnement Claude et agents tiers

Chronologie 2026 telle que rapportée par la note, **non revérifiée** : 9 janvier, blocage technique silencieux des tokens OAuth d'abonnement hors outils officiels → 17-20 février, formalisation dans la doc et les Consumer Terms (épisode OpenClaw) → 4 avril, application pleine → 13-14 mai, annonce de crédits Agent SDK séparés (20 $ Pro / 100 $ Max 5x / 200 $ Max 20x) pour le 15 juin → **15 juin, changement suspendu le jour même de son entrée en vigueur**. État rapporté au 2 août 2026 : l'Agent SDK, `claude -p` et les applications tierces puisent dans les limites de l'abonnement, le crédit séparé n'existant pas. **Daté par construction : à revérifier avant toute décision.**

### La règle qui survivra aux CGU

La ligne tracée sépare l'usage *« ordinaire et individuel »* du **routage de requêtes d'autrui** via des identifiants Free/Pro/Max. Un agent en `owner-only` consomme **votre** abonnement **pour vous** ; un agent en `anyone` dans un canal partagé fait passer les requêtes de vos collègues par votre compte, et c'est le moment de basculer sur une clé API facturée à l'usage. La distinction n'est pas juridique mais **architecturale : qui consomme, et pour le compte de qui** — à trancher à la conception.

C'est l'**ambient authority** de [[valente-zalewski-beyond-zero-enterprise-security-ai-era-2026-07-20]] transposée de l'autorisation à la facturation : un agent qui hérite des permissions de son humain et agit pour le compte d'un tiers pose le même problème de frontière, qu'il s'agisse d'accéder à un document ou de consommer un quota.

### Consigne de graphe issue de cette fiche

**Ne jamais créer d'entité `ACP`.** Les trois entités canoniques sont **Agent Client Protocol**, **Agentic Commerce Protocol** et **Agent Communication Protocol** ; « ACP » ne doit apparaître que comme alias explicitement rattaché à l'une d'elles.
