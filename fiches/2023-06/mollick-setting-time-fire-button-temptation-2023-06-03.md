# mollick-setting-time-fire-button-temptation-2023-06-03

## Veille
Crise de sens au travail - Bouton "Help me write" - Setting time on fire - Signaux d'effort - Lettres de recommandation IA - Ethan Mollick - One Useful Thing

## Titre Article
Setting time on fire and the temptation of The Button

## Date
2023-06-03

## URL
https://www.oneusefulthing.org/p/setting-time-on-fire-and-the-temptation

## Keywords
The Button, Help me write, Google Docs, Bard, GPT-4, crise de sens, setting time on fire, signaling, lettres de recommandation, performance reviews, meaningless work, mere ceremony, job satisfaction, busy-work, adoption IA, intégration outils, ancrage cognitif, productivité, quality vs effort

## Authors
Ethan Mollick

## Ton
**Profil** : Thought leadership académique, première personne réflexive, registre analytique-provocateur, niveau intermédiaire accessible.

**Description** : Mollick (professeur à Wharton) adopte une voix de leader d'opinion provocateur explorant les implications philosophiques et organisationnelles de l'IA générative au travail. La structure narrative conceptuelle (« Setting time on fire », « The Button », « crise de sens ») illustre un storytelling académique accessible au grand public. Le langage du chercheur en comportement organisationnel (signalement, « mere ceremony », recherche sur la satisfaction au travail) mêlé à des exemples concrets (lettres de recommandation, évaluations de performance) crée un pont entre théorie et pratique. Le ton, à la fois alarmé et optimiste, est typique de Mollick, qui équilibre des inquiétudes réelles avec des issues émancipatrices. L'aveu personnel (« I am still doing all my letters the old-fashioned way, but I wonder whether that will ultimately do my students a disservice ») révèle une confrontation authentique aux dilemmes éthiques. Format caractéristique des leaders d'opinion sur Substack (One Useful Thing) combinant rigueur académique et prose accessible, visant un large public professionnel en quête de sens face aux transformations technologiques.

## Pense-betes
- **"The Button"** : Google Docs "Help me write" - tentation irrésistible de générer premiers drafts avec IA
- **Microsoft Office intégrera bientôt** même capacité - adoption massive inévitable
- **Setting time on fire** : travail chronophage by design comme signal d'effort et d'engagement
- **Lettres de recommandation IA** : GPT-4 produit lettres excellentes en secondes vs 10x temps humain
- **Dilemme moral** : lettre IA peut être meilleure que lettre humaine médiocre - hurts students by NOT using AI?
- **Ancrage cognitif** : premiers drafts IA influencent travail final même si complètement réécrit
- **92% personnes** pensent leur job socialement utile - sens du travail crucial
- **Mere ceremony** : tâches organisationnelles sans réelle utilité - IA expose et amplifie
- **AI-to-AI workflows** : documents générés par IA envoyés à boîtes email IA - boucle sans humains
- **Crisis of meaning** : quand qualité output n'est plus signal de thoughtfulness/effort/time
- **Recherche Noy & Zhang** : utilisateurs IA enjoy work more, feel better able use talents
- **Documents comme signaux** : performance reviews, strategic memos, college essays, grants, speeches
- **Centuries-old approaches** filtering et signalling perdent valeur avec IA
- **Empowering outcome possible** : stripping away meaningless work libère workers, réduit inefficiencies
- **Redesign work nécessaire** : leaders doivent repenser travail pour monde "AI-haunted"
- **Boschian nightmare** métaphore : tâches sans sens comme enfer hiéronymien

## RésuméDe400mots

Ethan Mollick explore la crise de sens imminente au travail provoquée par l'intégration de l'IA générative dans les outils quotidiens, symbolisée par "The Button" - le bouton "Help me write" de Google Docs utilisant Bard, bientôt dans Microsoft Office avec GPT-4.

**La tentation du bouton**

Face à la page blanche, les gens appuieront sur The Button. L'adoption devient triviale quand l'IA s'intègre aux outils familiers (comme Adobe Photoshop). Étudiants, managers, professeurs, scientifiques - tous utiliseront ce bouton pour générer premiers drafts d'essays, emails, rapports, feedback, grants. L'ancrage cognitif implique que même les drafts complètement réécrits porteront une empreinte IA.

**Setting time on fire**

Mollick introduit le concept central : beaucoup de travail est chronophage by design. Une lettre de recommandation professorale prend du temps car cet effort signale le soutien réel au candidat. "We are setting our time on fire to signal to others that this letter is worth reading." Mais avec The Button, une lettre excellente se génère en secondes.

Il démontre avec un exemple concret : un prompt minimal produit via GPT-4 une lettre de recommandation "better than most letters I receive", répondant point-par-point aux exigences du poste avec détails plausibles (quoique inventés, mais facilement corrigibles via interaction IA). Le dilemme moral émerge : utiliser l'IA pourrait mieux servir l'étudiant qu'une lettre humaine médiocre.

**Documents devenus signaux vides**

Performance reviews, strategic memos, college essays, grants, speeches - tous ces documents où l'output écrit signale thoughtfulness et temps investi perdent leur valeur. Avec AI-to-AI workflows (documents générés par IA envoyés à boîtes email IA), les tâches deviennent "mere ceremony" - rituels organisationnels sans utilité réelle.

**Crise de sens**

92% des travailleurs considèrent leur job socialement utile. La recherche montre que l'IA augmente initialement job satisfaction - les pires tâches disparaissent, permettant focus sur meaningful work. Mais quand documents significatifs deviennent faciles à outsourcer, ou quand on réalise qu'aucun humain ne lit vraiment nos rapports, le sens s'évapore. Centuries-old approaches de filtering et signalling via effort écrit perdent toute valeur.

**Opportunité émancipatrice**

Mollick voit potentiellement un outcome empowering : libération du busy-work et mere ceremony. Ne plus "set our time on fire" comme signal. Mais cela requiert thoughtfulness organisationnelle - leaders redesigning work pour le monde "AI-haunted". Les incentives sont énormes : éliminer meaningless work réduit burden workers tout en supprimant inefficiencies.

**Urgence**

Cette transformation ne nécessite aucune IA plus avancée que celle existant en juin 2023. C'est déjà en cours dans l'éducation (essays AI-generated). Sans réflexion proactive maintenant, multiples crises de sens frapperont diverses professions alors que The Button se généralise.

## GrapheDeConnaissance

### Triples

| Sujet | Type Sujet | Prédicat | Objet | Type Objet | Confiance | Temporalité | Source |
|-------|-----------|----------|-------|-----------|-----------|-------------|--------|
| Ethan Mollick | PERSONNE | publie | Setting time on fire and the temptation of The Button | DOCUMENT | 0.99 | STATIQUE | déclaré_article |
| Google | ORGANISATION | a_créé | Help me write | TECHNOLOGIE | 0.99 | STATIQUE | déclaré_article |
| Help me write | TECHNOLOGIE | utilise | Bard | TECHNOLOGIE | 0.98 | STATIQUE | déclaré_article |
| Microsoft Office | TECHNOLOGIE | utilise | GPT-4 | TECHNOLOGIE | 0.95 | DYNAMIQUE | déclaré_article |
| GPT-4 | TECHNOLOGIE | permet | génération de lettres de recommandation en secondes | CONCEPT | 0.97 | STATIQUE | déclaré_article |
| The Button | CONCEPT | permet | crise de sens | CONCEPT | 0.95 | ATEMPOREL | inféré |
| setting time on fire | CONCEPT | est_instance_de | signal d'effort | CONCEPT | 0.97 | ATEMPOREL | déclaré_article |
| IA générative | TECHNOLOGIE | remplace | signal d'effort écrit | CONCEPT | 0.94 | DYNAMIQUE | inféré |
| Noy and Zhang | PERSONNE | publie | recherche satisfaction travail IA | DOCUMENT | 0.90 | STATIQUE | déclaré_article |
| utilisation IA au travail | METHODOLOGIE | améliore | satisfaction au travail | CONCEPT | 0.88 | DYNAMIQUE | déclaré_article |
| mere ceremony | CONCEPT | est_basé_sur | tâches organisationnelles sans utilité | CONCEPT | 0.92 | ATEMPOREL | déclaré_article |
| IA générative | TECHNOLOGIE | réduit | valeur des documents comme signaux | CONCEPT | 0.93 | DYNAMIQUE | déclaré_article |
| Ethan Mollick | PERSONNE | prédit | crise de sens au travail | CONCEPT | 0.96 | DYNAMIQUE | déclaré_article |
| Adobe Photoshop | TECHNOLOGIE | utilise | IA générative | TECHNOLOGIE | 0.92 | STATIQUE | déclaré_article |
| Ethan Mollick | PERSONNE | recommande | redesign du travail par les leaders pour le monde IA | CONCEPT | 0.88 | ATEMPOREL | déclaré_article |

### Entités

| Entité | Type | Attribut | Valeur | Action |
|--------|------|----------|--------|--------|
| Ethan Mollick | PERSONNE | affiliation | Wharton / One Useful Thing Substack | AJOUT |
| Help me write | TECHNOLOGIE | catégorie | Bouton IA intégré à Google Docs | AJOUT |
| Bard | TECHNOLOGIE | éditeur | Google | AJOUT |
| GPT-4 | TECHNOLOGIE | éditeur | OpenAI | AJOUT |
| Microsoft Office | TECHNOLOGIE | catégorie | Suite bureautique | AJOUT |
| Adobe Photoshop | TECHNOLOGIE | catégorie | Outil de retouche photo | AJOUT |
| The Button | CONCEPT | catégorie | Métaphore d'accès trivial à l'IA générative | AJOUT |
| setting time on fire | CONCEPT | catégorie | Signalement d'effort par le temps consacré | AJOUT |
| mere ceremony | CONCEPT | catégorie | Tâche organisationnelle sans utilité réelle | AJOUT |
| crise de sens | CONCEPT | catégorie | Perte de sens du travail face à l'IA | AJOUT |
| Noy and Zhang | PERSONNE | catégorie | Chercheurs en économie du travail et IA | AJOUT |
| lettre de recommandation | CONCEPT | catégorie | Document signal d'effort et de soutien | AJOUT |
