...
## Keywords
Boris Cherny, Anthropic, Claude Code, Sequoia, Lauren Reader, Asia, coding is solved, Claude code psychosis, /loop slash command, routines server-side, cron repeat jobs, agents in loops, multi-session iOS workflow, exponential growth Opus 4 May 2025, Anthropic Labs incubator late 2024, Mike Krieger round 2, MCP desktop app team disbanded, pre-PMF "build for the next model", TypeScript React on-distribution, 100% code generated, few dozen PRs daily, 150 PRs single day record, cross-disciplinary generalists, every team member codes, SaaS apocalypse no, Hamilton Helmer 7 Powers, switching costs decline, process power decline, network effects scale economies cornered resources persist, 10x more startups disrupting, printing press analogy 1400s, literacy 10% to 70%, books 100x cheaper, "best person to write accounting software is an accountant", harness less important as model improves, prompt injection static verification permission modes, model just figures it out, agent-to-agent Slack communication, no manually written code Anthropic, MCP for knowledge work, computer use catchall, Claude Design coming, model vs harness 50-50

## Authors
Boris Cherny (créateur de Claude Code, Anthropic) interviewé par Lauren Reader (Sequoia) avec introduction d'Asia (Sequoia).

## Ton
**Profil** : Interview live au format conversationnel devant une audience de fondateurs/investisseurs/builders Sequoia (*"as a roomful of builders"*, applaudissements, sondages à main levée à plusieurs reprises). Format hybride keynote-Q&A avec 6 questions audience. Public présent : opérateurs, fondateurs, investisseurs early-stage Sequoia ; public diffusé : la communauté tech mondiale via YouTube Sequoia Capital. Cherny n'est plus à présenter dans cette audience — l'intro est essentiellement comique (*"Boris is the father of Claude Code"*, *"Claude code psychosis"*).

**Style** : Cherny parle décontracté, première personne, hésitations naturelles assumées (*"like"*, *"you know"*), vocabulaire de bâtisseur plutôt que d'évangéliste. Il alterne entre **anecdote chiffrée** (*"there was a day last week I did like 150 PRs in a day. That was a record"*), **récit d'origine vécue** (la genèse Claude Code racontée comme un mini-roman startup), **vision audacieuse** (*"coding is solved"*, *"SaaS apocalypse"* analyse) et **autodérision** (*"My team lovingly says I have Claude code psychosis"*, *"It's extremely embarrassing, so please don't search it"* à propos d'un guide BASIC TI-83 Plus écrit au collège).

**Métaphores travaillées** :
- ***"Product overhang"*** : *"the model can do all the stuff that no product has yet captured"* — concept central pour comprendre la stratégie Claude Code (build for the next model).
- ***"Coding is solved"*** : déclaration provocatrice à valeur d'aphorisme.
- ***"Loops are the future"*** : pivot conceptuel sur la nouvelle primitive d'architecture agentique.
- ***Analogie de la presse de Gutenberg*** (1400s, 10% littéracie → 70%, livre 100× moins cher en 50 ans) — la métaphore historique la plus structurante de l'intervention. *"Software will be similarly democratized, but faster than 50 years."*
- ***"The best person to write accounting software is not an engineer, it's a really good accountant"*** — phrase-pivot sur la démocratisation et le rééquilibrage domaine vs codage.

**Position épistémique** : Cherny refuse à la fois la prudence excessive (*"coding is solved"* est une déclaration radicale) et le triomphalisme béat (il pose des limites : *"this is not the case everywhere — there's very big complicated codebases, weird languages the model isn't good at yet"*). Il assume la **prédiction** comme exercice (*"I'm here to make predictions, so I'll try to make some"*) et structure ses prédictions par **frameworks établis** (Helmer's 7 Powers) plutôt que par speculation libre.

**Autorité** : construite par (a) la position institutionnelle (créateur Claude Code, lead du produit le plus discuté de l'écosystème agentique), (b) la **pratique radicale dogfoodée** (100% code généré, 150 PRs/jour record, setup iOS exclusif), (c) l'humilité narrative (récit du *"it just really didn't work for the first 6 months"*), (d) le rapport de force inversé avec l'audience (*"Show of hands, who here uses Claude Code?"* — la majorité des mains se lèvent, le speaker n'a plus à se légitimer).
...
### Histoire de Claude Code (récit nouveau)
...
- **Pari pré-PMF assumé** : *"essentially we were trying to build this thing that was pre-PMF, and we knew that it wouldn't have PMF for 6 months because we were building for the next model."*
...
- **Codebase Claude Code** :
  ...
  - *"Pretty simple. There's nothing really complicated."*
...
- **Limites assumées** : *"this is not the case everywhere. There's very big complicated codebases. There's weird languages the model isn't good at yet. Usually the answer is just wait for the next model."*

### Setup personnel actuel (mises à jour)
...
- **4.7 démarre des loops spontanément** : *"I'll tell it 'pull this data query.' And it's like 'Hey, I noticed the data is changing over time. I'll start a loop and I'll give you a report every 30 minutes.' And I'm like 'Great. Can you send it to me over Slack?' And then it uses the Slack MCP to do that."*

### Vision sur les équipes
...
- **Sur la team Claude Code** :
  ...
  - *"They're specialist in something, but now also everyone's just coding."*

### Vision SaaS / disruption
...
### Analogie pivot — Presse de Gutenberg
...
- **Phrase-pivot mémorable** : *"The best person to write accounting software, I think maybe even today, is not an engineer, it's a really good accountant. Because they know the domain really well, and coding is the easy part. It's knowing the domain that's the hard part."*

### Modèle vs harness (Q&A)
...
- **Dans 1 an** : *"the model will be much better aligned. And so all the safety mechanisms that we have today around prompt injection and static verification of commands and permission modes, human in the loop, all this kind of stuff is just going to be less important because the model will just do the right thing."*

### Démocratisation : tous codeurs comme on sait envoyer un SMS

- *"I think it's going to be a skill like — yeah, like I know how to send a text message."*
- Référence forte au modèle de la presse de Gutenberg ci-dessus.

### Pratiques internes Anthropic
...
- *"All this stuff just doesn't matter that much. It could be MCPs, APIs, just some sort of programmatic access — to the model, it's just tokens."*
...
- **Quatrième fiche Cherny** dans le dossier après :
  - **cherny-yc-lightcone-claude-code-origin-story-2026-02** (genèse YC Lightcone)
  - **cherny-claude-code-10-tips-team-x-2026-02-01** (10 conseils X)
  - **nunez-cherny-claude-code-workflow-venturebeat-2026-01-05** (workflow VentureBeat)
  - **wu-cherny-use-claude-code-builders-every-2025-10-29** (Wu/Cherny Every)
  - Plus la fiche **anthropic-claude-code-quality-postmortem-2026-04-23** dont Cherny est un des auteurs implicites.
- **Apport spécifique de cette fiche** vs les antérieures :
  - **Setup iOS exclusif** = nouveau (vs setup CLI/desktop des fiches précédentes).
  - **`/loop` + Routines** = nouvelle primitive d'architecture agentique présentée publiquement.
  - **Récit "Anthropic Labs incubator + Mike Krieger round 2"** = nouveau détail organisationnel.
  - **Inflection Opus 4 mai 2025** = datation précise du décollage.
  - **"100% code généré depuis octobre/novembre 2025"** = datation personnelle.
  - **Analogie Gutenberg** + **"the best accountant writes the best accounting software"** = phrases-pivots mémorables.
  - **"7 Powers" Helmer reordering** = analyse SaaS structurée.
...
- **Convergence majeure** :
  ...
  - **"Coding is solved" + 100% code généré + 150 PRs/jour** rejoint **Curran/Intercom 3× R&D productivity** (2026-04-16, 93,6% PRs agent-driven), **Stripe Minions** (2026-02-09 et 2026-02-19), **StrongDM Software Factory** (2026-02-06).
...
| Anthropic dogfood doctrine | CONCEPT | description | "We have no more manually written code anywhere at the company. All SQL written by models. Same models as everyone else." Avantage Anthropic = organisationnel, pas technologique | AJOUT |
