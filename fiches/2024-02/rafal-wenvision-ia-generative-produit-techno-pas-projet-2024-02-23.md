...
|--------|------|----------|--------|--------|
| Olivier Rafal | PERSONNE | rôle | Consulting Director Strategy, WeNvision | AJOUT |
| WeNvision | ORGANISATION | secteur | Cabinet de conseil FR (stratégie / transformation IT) | AJOUT |
| CIO-Online | ORGANISATION | secteur | Média IT pour DSI (rubrique Tribune) | AJOUT |
| L'IA générative est plus une affaire de produit technologique qu'un projet d'IA | DOCUMENT | catégorie | Tribune d'opinion / cadre de décision DSI | AJOUT |
| produit vs projet | CONCEPT | définition | Traiter la plateforme GenAI comme un produit financé en flux, pas un projet ponctuel | AJOUT |
| foundation model | CONCEPT | coût | Plusieurs mois + millions d'euros + données massives | AJOUT |
| BloombergGPT | TECHNOLOGIE | catégorie | LLM financier créé par Bloomberg sur ses données propriétaires | AJOUT |
| plateforme d'IA générative | TECHNOLOGIE | architecture | Orchestrateurs + API = indépendance modèle (anti lock-in) | AJOUT |
| financement en flux | CONCEPT | principe | Budget mensuel continu plutôt qu'investissement ponctuel | AJOUT |
| shadow AI | CONCEPT | risque | Usage non gouverné né de la démocratisation de la GenAI | AJOUT |
| agents Langchain | TECHNOLOGIE | rôle | Gèrent une partie des décisions (changement de paradigme) | AJOUT |
