# Superpowers

> **Type** : TECHNOLOGIE | 17 relations | 2 fiches sources

## Attributs

- **catégorie** : Framework de skills agentiques pour agents de codage
- **licence** : MIT
- **étoiles GitHub** : 130 000+

## Relations (comme sujet)

### est_basé_sur

- YAGNI et DRY (CONCEPT) — 0.90, ATEMPOREL
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]

### mesure

- « moins de 2k tokens injectés au démarrage » (MESURE) — 0.90, DYNAMIQUE
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
- « 130k+ étoiles GitHub » (MESURE) — 0.85, DYNAMIQUE
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]

### permet

- dispatching vers sous-agents (METHODOLOGIE) — 0.93, STATIQUE
  - [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]
- création automatique de git worktrees (CONCEPT) — 0.92, STATIQUE
  - [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]

### s_applique_à

- [[kb/Claude-Code\|Claude Code]] (TECHNOLOGIE) — 0.98, DYNAMIQUE
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
- [[kb/Cursor-technologie\|Cursor]] (TECHNOLOGIE) — 0.90, DYNAMIQUE
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]

### utilise

- [[kb/skills-concept\|skills]] (CONCEPT) — 0.97, DYNAMIQUE
  - [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]
- fichiers SKILL.md (CONCEPT) — 0.95, ATEMPOREL
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
- [[kb/_entites-mineures#RED-GREEN-TDD\|RED/GREEN TDD]] (METHODOLOGIE) — 0.95, STATIQUE
  - [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]
- TDD rouge/vert strict (METHODOLOGIE) — 0.95, ATEMPOREL
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
- [[kb/_entites-mineures#workflow-Brainstorm-Plan-Implement\|workflow Brainstorm-Plan-Implement]] (METHODOLOGIE) — 0.95, STATIQUE
  - [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]
- workflow brainstorming avant code (METHODOLOGIE) — 0.95, ATEMPOREL
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
- mémoire conversationnelle SQLite (TECHNOLOGIE) — 0.88, DYNAMIQUE
  - [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]

## Relations (comme objet)

- [[kb/Jesse-Vincent\|Jesse Vincent]] **a_créé** → Superpowers — 0.98
- [[kb/_entites-mineures#Prime-Radiant\|Prime Radiant]] **a_créé** → Superpowers — 0.95
- Skills communautaires **améliore** → Superpowers — 0.85

## Fiches sources

- [[fiches/2025-10/superpowers-skills-coding-agents-vincent-2025-10-09\|Superpowers: How I'm using coding agents in October 2025]]
- [[fiches/2026-04/vincent-superpowers-agentic-skills-framework-github-2026-04-02\|Superpowers: An agentic skills framework & software development methodology that works]]
