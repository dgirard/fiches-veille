# Every

> **Type** : ORGANISATION | 9 relations | 5 fiches sources

## Attributs

- **attribut** : 15 personnes, 4 produits, 6 business units
- **produits** : Cora, Spiral, Sparkle, Monologue + incubations
- **secteur** : Médias / Logiciels IA

## Relations (comme sujet)

### a_été_transformé_par

- [[kb/Claude-Code\|Claude Code]] (TECHNOLOGIE) — 0.92, STATIQUE
  - [[fiches/2025-10/wu-cherny-use-claude-code-builders-every-2025-10-29\|Claude Code - Cat Wu & Boris Cherny - Antfooding - Plan Mode - Subagents - Slash Commands - Stop Hooks - Settings.json - Dangerous Mode - AI & I Podcast - Every]]

### héberge

- [[kb/_entites-mineures#planning-system-open-source\|planning system open-source]] (TECHNOLOGIE) — 0.93, DYNAMIQUE
  - [[fiches/2025-11/klaassen-teach-ai-think-senior-engineer-every-2025-11-07\|8 stratégies planification IA - Research agents parallèles - Codebase grounding - Git history - Vibe prototyping - Style agents - Compounding engineering - Every Source Code - Kieran Klaassen]]

### pratique

- [[kb/compound-engineering\|compound engineering]] (METHODOLOGIE) — 0.98, DYNAMIQUE
  - [[fiches/2025-12/shipper-klaassen-compound-engineering-every-agents-2025-12-11\|Compound Engineering : processus 4 étapes (Plan, Work, Assess, Compound) pour équipes qui codent avec agents IA - Every]]

### produit

- [[kb/_entites-mineures#Koala,-Spiral,-Monologue\|Koala, Spiral, Monologue]] (TECHNOLOGIE) — 0.90, DYNAMIQUE
  - [[fiches/2025-11/shipper-every-building-ai-native-company-2025-11-23\|Every - Compounding Engineering - Single Developer Products - AI Native - Tacit Knowledge Sharing]]

### publie

- [[kb/_entites-mineures#plugin-compound-engineering-Claude-Code\|plugin compound engineering Claude Code]] (TECHNOLOGIE) — 0.96, STATIQUE
  - [[fiches/2025-12/shipper-klaassen-compound-engineering-every-agents-2025-12-11\|Compound Engineering : processus 4 étapes (Plan, Work, Assess, Compound) pour équipes qui codent avec agents IA - Every]]

## Relations (comme objet)

- [[kb/Dan-Shipper\|Dan Shipper]] **a_fondé** → Every — 0.98
- [[kb/Dan-Shipper\|Dan Shipper]] **dirige** → Every — 0.98
- [[kb/Cora\|Cora]] **fait_partie_de** → Every — 0.98
- [[kb/Kieran-Klaassen\|Kieran Klaassen]] **publie_dans** → Every — 0.98

## Fiches sources

- [[fiches/2025-11/klaassen-stop-coding-start-planning-every-2025-11-06\|Planification vs Vibe Coding - Compounding Engineering - Three Fidelities - AI Agents - Cora Email Bankruptcy - Plans teach systems - Every Source Code]]
- [[fiches/2025-11/klaassen-teach-ai-think-senior-engineer-every-2025-11-07\|8 stratégies planification IA - Research agents parallèles - Codebase grounding - Git history - Vibe prototyping - Style agents - Compounding engineering - Every Source Code - Kieran Klaassen]]
- [[fiches/2025-11/shipper-every-building-ai-native-company-2025-11-23\|Every - Compounding Engineering - Single Developer Products - AI Native - Tacit Knowledge Sharing]]
- [[fiches/2025-12/shipper-klaassen-compound-engineering-every-agents-2025-12-11\|Compound Engineering : processus 4 étapes (Plan, Work, Assess, Compound) pour équipes qui codent avec agents IA - Every]]
- [[fiches/2025-10/wu-cherny-use-claude-code-builders-every-2025-10-29\|Claude Code - Cat Wu & Boris Cherny - Antfooding - Plan Mode - Subagents - Slash Commands - Stop Hooks - Settings.json - Dangerous Mode - AI & I Podcast - Every]]
