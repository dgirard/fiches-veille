# dette technique

> **Type** : CONCEPT | 5 relations | 4 fiches sources

## Attributs

- **cause principale** : usage non encadré de l'IA agentique
- **description** : Accumulation de code de faible qualité sur le long terme
- **risque** : Amplifié par IA sans discipline de test
- **risque_associé** : supervision insuffisante du code IA

## Relations (comme objet)

- [[kb/Vibe-Coding\|vibe coding]] **génère** → dette technique — 0.95
- [[kb/_entites-mineures#code-review-IA\|code review IA]] **réduit** → dette technique — 0.88
- [[kb/Compounding-Engineering-Plugin\|Compounding Engineering Plugin]] **réduit** → dette technique — 0.88
- [[kb/agents-IA\|agents IA]] **génère** → dette technique — 0.85
- [[kb/_entites-mineures#revue-de-code-rigoureuse\|revue de code rigoureuse]] **réduit** → dette technique — 0.85

## Fiches sources

- [[fiches/2025-09/ia-sdlc-cutting-hype-aijournal-2025-09-15\|IA dans le cycle de vie logiciel - Qualité vs vitesse - Assurance qualité systématique - AI Journal]]
- [[fiches/2024-10/kent-beck-vibe-coding-tdd-ai-assisted-dev-linkedin-2024-10-17\|Kent Beck - Vibe Coding - TDD - AI-assisted development - Software craftsmanship - LinkedIn - Agile methodology]]
- [[fiches/2026-02/martin-bmad-method-urbanisme-ia-agentique-sdlc-2026-02-04\|Méthode BMAD : cadre structuré pour intégrer l'IA agentique dans le SDLC - Tech Away]]
- [[fiches/2025-11/vibe-coding-vs-ai-assisted-engineering-osmani-2025-11-01\|Vibe coding vs AI-assisted engineering - Addy Osmani - Software development - Engineering principles - LinkedIn]]
