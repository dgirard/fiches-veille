# Mistral AI CLI

> **Type** : TECHNOLOGIE | 7 relations | 1 fiches sources

## Attributs

- **catégorie** : Agent de codage CLI open source
- **dépôt** : github.com/garciamathias/mistral-cli

## Relations (comme sujet)

### est_alternative_à

- [[kb/Claude-Code\|Claude Code]] (TECHNOLOGIE) — 0.99, STATIQUE
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

### est_basé_sur

- [[kb/Mistral-AI\|Mistral AI]] (ORGANISATION) — 0.96, STATIQUE
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

### est_publié_sur

- [[kb/_entites-mineures#GitHub\|GitHub]] (TECHNOLOGIE) — 0.98, STATIQUE
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

### manque

- [[kb/_entites-mineures#intégration-abonnement-professionnel-Mistral\|intégration abonnement professionnel Mistral]] (CONCEPT) — 0.88, DYNAMIQUE
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

### représente

- [[kb/_entites-mineures#souveraineté-technologique-européenne\|souveraineté technologique européenne]] (CONCEPT) — 0.85, ATEMPOREL
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

### s_oppose_à

- [[kb/Cursor\|Cursor]] (TECHNOLOGIE) — 0.80, DYNAMIQUE
  - [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]

## Relations (comme objet)

- [[kb/_entites-mineures#Mathias-Garcia\|Mathias Garcia]] **a_créé** → Mistral AI CLI — 0.97

## Fiches sources

- [[fiches/2025-07/mistral-cli-european-alternative-claude-code-garcia-2025-07-01\|Mistral AI CLI - Alternative européenne Claude Code - Open source - Mathias Garcia - HEC Paris - LinkedIn]]
