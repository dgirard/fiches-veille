# Atlassian

> **Type** : ORGANISATION | 8 relations | 1 fiches sources

## Attributs

- **secteur** : Éditeur logiciel (collaboration, ingénierie)

## Relations (comme sujet)

### mesure

- « +19 % de PR mergées par mois pour les dépôts adoptant Rovo Dev » (MESURE) — 0.95, STATIQUE
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]
- « 2-3 heures gagnées par développeur et par semaine » (MESURE) — 0.93, STATIQUE
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

### publie

- The AI-native SDLC is paying off (DOCUMENT) — 0.97, STATIQUE
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

### recommande

- « démarrer par une équipe (3-5 ingénieurs), pas un individu, et mesurer 2-3 mois après » (AFFIRMATION) — 0.92, ATEMPOREL
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]
- « passer des métriques d'usage (tokens) aux métriques d'impact » (AFFIRMATION) — 0.90, ATEMPOREL
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

### surpasse

- « clients externes (1,35× plus de PR assistées mergées) » (MESURE) — 0.85, STATIQUE
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

## Relations (comme objet)

- [[kb/_entites-mineures#Fan-Jiang\|Fan Jiang]] **travaille_chez** → Atlassian — 0.95
- [[kb/_entites-mineures#Robbie-Geoghegan\|Robbie Geoghegan]] **travaille_chez** → Atlassian — 0.95

## Fiches sources

- [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]
