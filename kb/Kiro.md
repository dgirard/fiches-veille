# Kiro

> **Type** : TECHNOLOGIE | 12 relations | 4 fiches sources

## Attributs

- **catégorie** : IDE + CLI d'AWS, standard de développement interne, sur Claude Sonnet/Opus
- **fonctionnalité** : Mode "Vibe" et mode "Spec" intégrés

## Relations (comme sujet)

### concurrence

- [[kb/Cursor-technologie\|Cursor]] (TECHNOLOGIE) — 0.97, DYNAMIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- [[kb/Windsurf-technologie\|Windsurf]] (TECHNOLOGIE) — 0.97, DYNAMIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]

### est_basé_sur

- [[kb/_entites-mineures#Code-OSS\|Code OSS]] (TECHNOLOGIE) — 0.99, STATIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]

### s_oppose_à

- [[kb/_entites-mineures#Amazon-Q-Developer\|Amazon Q Developer]] (TECHNOLOGIE) — 0.88, ATEMPOREL
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]

### utilise

- [[kb/_entites-mineures#Claude-Sonnet\|Claude Sonnet]] (TECHNOLOGIE) — 0.98, DYNAMIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- [[kb/_entites-mineures#spec-coding\|spec coding]] (METHODOLOGIE) — 0.98, STATIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- modèle tarifaire usage-based (CONCEPT) — 0.97, DYNAMIQUE
  - [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- Claude Opus (TECHNOLOGIE) — 0.90, DYNAMIQUE
  - [[fiches/2026-04/ifttd-351-aws-summit-julien-lepine-2026-04-08\|IFTTD #351 - AWS Summit : Rester aux commandes des agents de code (avec Julien Lépine)]]

## Relations (comme objet)

- [[kb/AWS\|AWS]] **publie** → Kiro — 0.99
- [[kb/AWS\|AWS]] **a_créé** → Kiro — 0.98
- [[kb/_entites-mineures#Amazon-Web-Services\|Amazon Web Services]] **utilise** → Kiro — 0.96
- [[kb/Cursor-technologie\|Cursor]] **converge_avec** → Kiro — 0.95

## Fiches sources

- [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- [[fiches/2025-09/fin-assistants-ia-bon-marche-infoworld-2025-09-15\|L'ère des assistants de codage IA bon marché pourrait être révolue]]
- [[fiches/2026-04/ifttd-351-aws-summit-julien-lepine-2026-04-08\|IFTTD #351 - AWS Summit : Rester aux commandes des agents de code (avec Julien Lépine)]]
- [[fiches/2025-09/vibe-coding-hot-summer-redmonk-2025-09-08\|The Endless Hot Vibe Code Summer]]
