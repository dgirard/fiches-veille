# ADLC Toolkit

> **Type** : TECHNOLOGIE | 4 relations | 1 fiches sources

## Attributs

- **catégorie** : Suite open-source de 18 outils npx zéro-dépendance pour le cycle ADLC
- **livraison** : github.com/voodootikigod/adlc, package @adlc/core (npm)

## Relations (comme sujet)

### est_basé_sur

- [[kb/ADLC\|ADLC]] (METHODOLOGIE) — 0.92, STATIQUE
  - [[fiches/2026-06/williams-adlc-7-built-with-the-lifecycle-2026-06-12\|The ADLC Toolkit]]

### utilise

- [[kb/_entites-mineures#@adlc-core\|@adlc/core]] (TECHNOLOGIE) — 0.92, STATIQUE
  - [[fiches/2026-06/williams-adlc-7-built-with-the-lifecycle-2026-06-12\|The ADLC Toolkit]]
- outils npx à exit codes déterministes (0 pass / 2 gate échoué) (TECHNOLOGIE) — 0.89, STATIQUE
  - [[fiches/2026-06/williams-adlc-7-built-with-the-lifecycle-2026-06-12\|The ADLC Toolkit]]

## Relations (comme objet)

- [[kb/Chris-Williams\|Chris Williams]] **a_créé** → ADLC Toolkit — 0.95

## Fiches sources

- [[fiches/2026-06/williams-adlc-7-built-with-the-lifecycle-2026-06-12\|The ADLC Toolkit]]
