# Compaction

> **Type** : CONCEPT | 4 relations | 2 fiches sources

> **Même entité, autre type** : [[kb/Compaction-methodologie\|Compaction (METHODOLOGIE)]]

## Attributs

- **définition** : Résumé automatique de la conversation remplaçant l'historique

## Relations (comme sujet)

### fait_partie_de

- [[kb/_entites-mineures#API-Anthropic\|API Anthropic]] (TECHNOLOGIE) — 0.92, STATIQUE
  - [[fiches/2026-02/trq212-anthropic-claude-code-prompt-caching-lessons-2026-02\|Lessons from Building Claude Code: Prompt Caching Is Everything]]

### réduit

- Historique de conversation (CONCEPT) — 0.95, ATEMPOREL
  - [[fiches/2026-04/thariq-claude-code-session-management-1m-context-2026-04-14\|Using Claude Code: Session Management & 1M Context]]

### résout

- [[kb/Context-Rot\|Context Rot]] (CONCEPT) — 0.90, ATEMPOREL
  - [[fiches/2026-03/trivedy-langchain-anatomy-agent-harness-2026-03-10\|The Anatomy of an Agent Harness]]

### utilise

- même system prompt et outils (CONCEPT) — 0.96, ATEMPOREL
  - [[fiches/2026-02/trq212-anthropic-claude-code-prompt-caching-lessons-2026-02\|Lessons from Building Claude Code: Prompt Caching Is Everything]]

## Fiches sources

- [[fiches/2026-04/thariq-claude-code-session-management-1m-context-2026-04-14\|Using Claude Code: Session Management & 1M Context]]
- [[fiches/2026-02/trq212-anthropic-claude-code-prompt-caching-lessons-2026-02\|Lessons from Building Claude Code: Prompt Caching Is Everything]]
