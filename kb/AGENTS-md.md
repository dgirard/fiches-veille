# AGENTS.md

> **Type** : TECHNOLOGIE | 6 relations | 3 fiches sources

## Attributs

- **adoption** : 60 000+ projets
- **catégorie** : Fichier de contexte persistant pour agents IA
- **guidelines** : Pilot's checklist, pas style guide. <60 lignes (HumanLayer). Earn each line.

## Relations (comme sujet)

### mesure

- « taux de réussite 100% » (MESURE) — 0.99, STATIQUE
  - [[fiches/2026-01/gao-vercel-agents-md-outperforms-skills-evals-2026-01-27\|AGENTS.md outperforms skills in our agent evals]]
- « adoption par 60 000+ projets open source » (MESURE) — 0.93, DYNAMIQUE
  - [[fiches/2025-12/openai-agentic-ai-foundation-linux-2025-12-09\|OpenAI co-founds the Agentic AI Foundation under the Linux Foundation]]

### surpasse

- [[kb/skills-technologie\|skills]] (TECHNOLOGIE) — 0.99, STATIQUE
  - [[fiches/2026-01/gao-vercel-agents-md-outperforms-skills-evals-2026-01-27\|AGENTS.md outperforms skills in our agent evals]]

## Relations (comme objet)

- [[kb/OpenAI\|OpenAI]] **publie** → AGENTS.md — 0.97
- [[kb/CLAUDE-md-technologie\|CLAUDE.md]] **est_variante_de** → AGENTS.md — 0.96
- [[kb/_entites-mineures#Ratchet-principle\|Ratchet principle]] **est_basé_sur** → AGENTS.md — 0.95

## Fiches sources

- [[fiches/2026-01/gao-vercel-agents-md-outperforms-skills-evals-2026-01-27\|AGENTS.md outperforms skills in our agent evals]]
- [[fiches/2025-12/openai-agentic-ai-foundation-linux-2025-12-09\|OpenAI co-founds the Agentic AI Foundation under the Linux Foundation]]
- [[fiches/2026-04/osmani-agent-harness-engineering-2026-04-19\|Agent Harness Engineering]]
