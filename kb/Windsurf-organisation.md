# Windsurf

> **Type** : ORGANISATION | 7 relations | 6 fiches sources

> **Même entité, autre type** : [[kb/Windsurf-technologie\|Windsurf (TECHNOLOGIE)]]

## Attributs

- **catégorie** : IDE assisté par IA

## Relations (comme sujet)

### fait_partie_de

- [[kb/_entites-mineures#Cognition\|Cognition]] (ORGANISATION) — 0.97, STATIQUE
  - [[fiches/2025-09/vibe-coding-hot-summer-redmonk-2025-09-08\|The Endless Hot Vibe Code Summer]]
- [[kb/Google\|Google]] (ORGANISATION) — 0.93, STATIQUE
  - [[fiches/2025-10/a16z-trillion-dollar-ai-dev-stack-2025-10-09\|The Trillion Dollar AI Software Development Stack]]

### observé_dans

- « précédent montrant que les fournisseurs de modèles peuvent restreindre l'accès plus vite qu'un acquéreur ne l'anticipe » (AFFIRMATION) — 0.87, STATIQUE
  - [[fiches/2026-04/ashley-futurum-spacex-cursor-2026-04-29\|Why SpaceX-Cursor Works for Both, and What It Means for Google, AWS, IBM]]

## Relations (comme objet)

- [[kb/Deepnote\|Deepnote]] **s_applique_à** → Windsurf — 0.98
- [[kb/Kiro\|Kiro]] **concurrence** → Windsurf — 0.97
- [[kb/Cursor-technologie\|Cursor]] **concurrence** → Windsurf — 0.88
- [[kb/Augmented-Craftsman\|Augmented Craftsman]] **utilise** → Windsurf — 0.87

## Fiches sources

- [[fiches/2025-10/a16z-trillion-dollar-ai-dev-stack-2025-10-09\|The Trillion Dollar AI Software Development Stack]]
- [[fiches/2026-04/ashley-futurum-spacex-cursor-2026-04-29\|Why SpaceX-Cursor Works for Both, and What It Means for Google, AWS, IBM]]
- [[fiches/2025-07/aws-kiro-specification-driven-agentic-ide-forbes-2025-07-15\|AWS Launches Kiro, A Specification-Driven Agentic IDE]]
- [[fiches/2025-11/deepnote-jupyter-successor-ai-first-github-2025-11-07\|Deepnote: the data notebook for the AI era]]
- [[fiches/2025-06/touilleur-express-ni-manager-ni-contributeur-2025-06-23\|Ni manager, ni contributeur individuel… - Le Touilleur Express]]
- [[fiches/2025-09/vibe-coding-hot-summer-redmonk-2025-09-08\|The Endless Hot Vibe Code Summer]]
