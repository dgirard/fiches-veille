# SDLC AI-native

> **Type** : METHODOLOGIE | 10 relations | 5 fiches sources

## Attributs

- **avertissement** : Un SDLC piloté par l'IA punit les mauvaises habitudes plus vite (amplifie les dysfonctionnements)
- **définition** : Cycle de dev compressé (prototypes + dogfooding) où Claude Code/Claude Tag écrit et relit la majorité du code ; mêmes étapes (Plan/Code/Test/Deploy/Monitor) que le SDLC classique
- **structure** : Boucle de six étapes (Plan, Design, Build, Test, Deploy, Maintain), chaque étape committant l'artefact que lit la suivante
- **étapes** : Requirements, Design/Architecture, Implementation, Testing/QA, Deployment, Maintenance, agents + humains

## Relations (comme sujet)

### est_variante_de

- [[kb/SDLC\|SDLC]] (METHODOLOGIE) — 0.92, ATEMPOREL
  - [[fiches/2026-08/claxton-anthropic-ai-native-sdlc-playbook-2026-08-21\|The AI-Native SDLC playbook: How to transform your software development lifecycle with AI—stage by stage]]
  - [[fiches/2026-07/clinton-anthropic-secure-ai-native-sdlc-2026-07-21\|How Anthropic secures its AI-native software development lifecycle]]
  - [[fiches/2026-06/hingel-augment-how-ai-changes-sdlc-six-stages-2026-06-08\|How AI Changes the SDLC: A Six-Stage Guide]]

### fait_partie_de

- partenariat humain-agent sur cinq étapes (CONCEPT) — 0.90, ATEMPOREL
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

### permet

- ancrage des gates de sécurité entre deux étapes nommées (CONCEPT) — 0.95, ATEMPOREL
  - [[fiches/2026-07/sfeir-anthropic-sdlc-ai-native-securise-2026-07-26\|Anthropic sécurise un SDLC où l'IA écrit 80 % du code : le cycle redevient le socle]]

### utilise

- artefact committé (CONCEPT) — 0.93, ATEMPOREL
  - [[fiches/2026-08/claxton-anthropic-ai-native-sdlc-playbook-2026-08-21\|The AI-Native SDLC playbook: How to transform your software development lifecycle with AI—stage by stage]]
- [[kb/_entites-mineures#Rovo-Dev\|Rovo Dev]] (TECHNOLOGIE) — 0.88, DYNAMIQUE
  - [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]

## Relations (comme objet)

- [[kb/_entites-mineures#loi-d'Amdahl\|loi d'Amdahl]] **s_applique_à** → SDLC AI-native — 0.96
- [[kb/SFEIR\|SFEIR]] **affine** → SDLC AI-native — 0.93
- [[kb/_entites-mineures#cycle-à-11-phases\|cycle à 11 phases]] **converge_avec** → SDLC AI-native — 0.92
- [[kb/_entites-mineures#intent-md\|intent.md]] **fait_partie_de** → SDLC AI-native — 0.92
- [[kb/Claude-entite\|Claude]] **observé_dans** → SDLC AI-native — 0.90

## Fiches sources

- [[fiches/2026-05/atlassian-ai-native-sdlc-paying-off-rovo-dev-2026-05-31\|The AI-native SDLC is paying off: 19% more PRs and 2–3 hours saved per developer per week]]
- [[fiches/2026-08/claxton-anthropic-ai-native-sdlc-playbook-2026-08-21\|The AI-Native SDLC playbook: How to transform your software development lifecycle with AI—stage by stage]]
- [[fiches/2026-07/clinton-anthropic-secure-ai-native-sdlc-2026-07-21\|How Anthropic secures its AI-native software development lifecycle]]
- [[fiches/2026-06/hingel-augment-how-ai-changes-sdlc-six-stages-2026-06-08\|How AI Changes the SDLC: A Six-Stage Guide]]
- [[fiches/2026-07/hohpe-decision-options-ia-2026-07-15\|Gregor Hohpe et le rôle de l'architecte à l'ère de l'IA]]
