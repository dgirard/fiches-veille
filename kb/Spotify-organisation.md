# Spotify

> **Type** : ORGANISATION | 5 relations | 3 fiches sources

> **Même entité, autre type** : [[kb/Spotify-technologie\|Spotify (TECHNOLOGIE)]]

## Attributs

- **secteur** : Streaming musical / Technologie ; publie son outillage agent interne sur engineering.atspotify.com

## Relations (comme sujet)

### a_créé

- [[kb/_entites-mineures#Portal-by-Spotify\|Portal by Spotify]] (TECHNOLOGIE) — 0.95, STATIQUE
  - [[fiches/2026-09/mazmanov-portal-spotify-shunt-claude-code-tokens-2026-09-03\|Portal by Spotify cut my Claude Code token usage by 90%]]
- [[kb/_entites-mineures#Honk\|Honk]] (TECHNOLOGIE) — 0.94, STATIQUE
  - [[fiches/2026-08/staples-gitlab-when-code-is-abundant-2026-08-24\|When code is abundant]]

### est_instance_de

- stade Marchandise (CONCEPT) — 0.87, DYNAMIQUE
  - [[fiches/2025-10/wardley-mapping-explique-guide-strategique-2025-10-01\|Votre nouveau super-pouvoir : voir le jeu dans son ensemble (Wardley Mapping Expliqué)]]

### utilise

- IA pour résolution incidents (TECHNOLOGIE) — 0.88, DYNAMIQUE
  - [[fiches/2025-11/reock-dx-leadership-ai-engineering-metrics-2025-11-23\|Effective Leadership in AI-Enhanced Organizations]]

## Relations (comme objet)

- [[kb/Dimitri-Mazmanov\|Dimitri Mazmanov]] **travaille_chez** → Spotify — 0.97

## Fiches sources

- [[fiches/2026-09/mazmanov-portal-spotify-shunt-claude-code-tokens-2026-09-03\|Portal by Spotify cut my Claude Code token usage by 90%]]
- [[fiches/2025-11/reock-dx-leadership-ai-engineering-metrics-2025-11-23\|Effective Leadership in AI-Enhanced Organizations]]
- [[fiches/2025-10/wardley-mapping-explique-guide-strategique-2025-10-01\|Votre nouveau super-pouvoir : voir le jeu dans son ensemble (Wardley Mapping Expliqué)]]
